...
### 症狀
連續6局以上開同一結果（全莊或全閒）。連續5局整副牌只允許出現一次，第二次起視為違規需修復。
...
### A2 模式（全花色 A、2）— 不修復，改用容許 + 重跑

> **核心差異：** A2 模式下所有 A 都是訊號牌、所有 2 都是訊號牌，64 張訊號牌導致大量訊號鏈（連續局都有 A/2），每局都強制下一局開莊，形成無法打破的連續莊。對調前兩張幾乎一定會產生新的訊號牌違規，因此**不使用對調方式修復**。
...
#### 原因
- A2 模式 64 張訊號牌佔牌靴比例極高，幾乎每局都有訊號牌
...
> **核心差異：** 無法用方法A（換掉訊號牌），因為所有 A 都是訊號牌、所有 2 都是訊號牌，換了還是訊號牌。
...
> **A2 模式關鍵：** 連續莊/閒不修復，靠容許規則 + 重跑處理。其餘違規中，訊號牌違規只能用「對調前兩張」，不能用換牌方式。
